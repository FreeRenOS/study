...
class MyApp(QtWidgets.QDialog):
    ...
    def __init__(self):
        ...
        self.thr_value2 = self.findChild(QtWidgets.QLabel, 'label_threshold_2')
        self.hSlider2 = self.findChild(QtWidgets.QSlider, 'hSlider_2')
        self.hSlider2.setTickPosition(QtWidgets.QSlider.TicksBelow)
        self.hSlider2.valueChanged.connect(self.hSliderChanged)
    ...
    def hSliderChanged(self):
        if self.filename !='':
            if  self.threshold_option == cv2.Canny :
                self.thr_value2.setText("임계값 :"+str(self.hSlider2.value()))
                self.thr_value.setText("임계값 :"+str(self.hSlider.value()))
                img_gray = cv2.cvtColor(self.img2, cv2.COLOR_BGR2GRAY)
                binary = cv2.Canny(img_gray, self.hSlider2.value(), self.hSlider.value())
                self.imgThr = binary
    ...
    def Connect_Detect(self):
        if self.count != 0:
            ...
            print(MomentList)
            # monent_arr = np.array(MomentList)
            # monent_arr = monent_arr[monent_arr[:,0].argsort()]
            
            df_Moment = pd.DataFrame(MomentList, columns=['x','y']) 
            
            df_Moment = df_Moment.sort_values(by=['x'])
            print(df_Moment)
            
            df_upMoment = df_Moment[df_Moment.y < y0+int(h0/2)] # 중심선 위에 있는 모멘트
            print(df_upMoment)
            df_downMoment = df_Moment[df_Moment.y > y0+int(h0/2)] # 중심선 아래에 있는 모멘트
            print(df_downMoment)

            for i in range(len(df_upMoment)):
                cv2.putText(outImg, str(i+1), (df_upMoment.iloc[i,0],df_upMoment.iloc[i,1]-30),cv2.FONT_HERSHEY_COMPLEX, 0.7, (255, 255, 127), 2)
            for i in range(len(df_downMoment)):
                cv2.putText(outImg, str(i+1), (df_downMoment.iloc[i,0],df_downMoment.iloc[i,1]-30),cv2.FONT_HERSHEY_COMPLEX, 0.7, (255, 255, 127), 2)
