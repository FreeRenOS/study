...
class MyApp(QtWidgets.QDialog):
    ...
    def processingImage(self, img_rgb, img_gray):
        ...
        if self.imgThr.ndim == 2 : # 그래이 영상
            ...
            ret1, img_gray = cv2.threshold(img_gray, self.hSlider.value(), 255, 0)

        ## 모폴로지 변환
        kernelsize = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
        ...
        ##관심영역 짜르고 네모영역 그리기
        # ret1, img_Roi = cv2.threshold(img_gray, 25, 255, cv2.THRESH_BINARY_INV)
        # kernelsize = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
        # img_Roi = cv2.morphologyEx(img_Roi, cv2.MORPH_CLOSE, kernelsize, iterations=1)
        # cv2.rectangle(img_Roi, (540, 830), (2055, 1140), (255, 0, 255), 2)
        ...
        # cv2.line(outImg, (x_min, y_min), (x_max, y_min), (255, 0, 255), 2)
        # cv2.line(outImg, (x_max, y_min), (x_max, y_max), (255, 0, 255), 2)
        # cv2.line(outImg, (x_max, y_max), (x_min, y_max), (255, 0, 255), 2)
        # cv2.line(outImg, (x_min, y_max), (x_min, y_min), (255, 0, 255), 2)
